...
def collate_fn(batch):
    inputs, leaf, joints = zip(*batch)
    inputs = pad_sequence(inputs, batch_first=True)
    leaf   = pad_sequence(leaf, batch_first=True)
    joints = pad_sequence(joints, batch_first=True)
    return inputs, leaf, joints
...
def main():
    ...
    processed_dir = base_dir / "finetune_data"
    ...
    best_model_path_1 = models_dir / "best_tunemodel.pth"
...
# -------------------------
    ...
    # -------------------------
# 2. Only fine-tune input and output projection layers
# -------------------------
    ...
    last_model_path = models_dir / "last_tune_model.pth"
    torch.save(model.state_dict(), last_model_path)

    norm_params_path = models_dir / "norm_tune_params.npz"
    np.savez(
        norm_params_path,
        acc_mean=acc_mean,
        acc_std=acc_std,
    ...
    print(f"Best model saved to: {best_model_path_1}")
